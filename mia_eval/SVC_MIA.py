...
def collect_prob(loader, model, device, classes, hook):
    if loader is None:
        return torch.zeros([0, 10]), torch.zeros([0])
    
    n = len(loader.dataset)
    prob = torch.empty((n, classes))
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            batch_size = x.size(0)
            logit = model(x)
            prob[idx:idx+batch_size] = F.softmax(logit, dim=-1).data
...
def SVC_MIA(shadow_train, target_train, target_test, shadow_test, model, device, classes, hook):
    shadow_train_prob, shadow_train_labels = collect_prob(shadow_train, model, device, classes, hook) #shadow_train_prob: (10000,10) 10000 samples, 10 classes
    shadow_test_prob, shadow_test_labels = collect_prob(shadow_test, model, device, classes, hook)

    target_train_prob, target_train_labels = collect_prob(target_train, model, device, classes, hook)
    target_test_prob, target_test_labels = collect_prob(target_test, model, device, classes, hook)
