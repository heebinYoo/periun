...
for i in range(3):
    retrain_state = torch.load(f'assets/optimal_{args.target}_unlearn_model/{args.method}_{dataset}_{arch}_model_{i}_unlearn_{unlearn_seed}.pth', weights_only=True)
    ...
    os.makedirs('assets/mia', exist_ok=True)
    csv_file_path = f'assets/mia/mia_results_{args.target}.csv'
    header = ['method', 'setup', 'unlearn_seed', 'init_seed', 'correctness_acc', 'confidence_acc', 'entropy_acc', 'm_entropy_acc', 'prob_acc']
